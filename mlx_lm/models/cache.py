...
class QuantizedRotatingKVCache(_RotatingKVCacheBase):
    ...
    @property
    def state(self):
        # keep/max_size/group_size/bits/_idx ride along in the same tuple as
        # keys/values (not a separate meta_state) -- save_prompt_cache's
        # map_data only ever serializes whatever .state returns, same
        # convention QuantizedKVCache already uses for its own scalars.
        if self.keys is None:
            keys = values = None
        elif self.offset < self.keys[0].shape[2]:
            keys = self._q_slice(self.keys, 0, self.offset)
            values = self._q_slice(self.values, 0, self.offset)
        else:
