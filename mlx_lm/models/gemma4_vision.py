...
class PatchEmbedder(nn.Module):
    ...
    def __call__(
        ...
        # The activation dtype, from a table quantization never touches: a
        # quantized input_proj's weight is packed uint32 (casting pixel values
        # in [-1, 1] to it zeroed them, and every image came out the same),
        # and in the mx/nv fp modes its scales are uint8.
        dtype = self.position_embedding_table.dtype
